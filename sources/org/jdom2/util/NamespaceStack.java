...
public final class NamespaceStack implements Iterable<Namespace> {
    ...
    public void push(Element element) {
        ...
        if (element.hasAdditionalNamespaces()) {
            for (Namespace next : element.getAdditionalNamespaces()) {
                if (next != namespace) {
                    checkNamespace = checkNamespace(arrayList, next, checkNamespace);
                }
            }
        }
        if (element.hasAttributes()) {
            for (Attribute namespace2 : element.getAttributes()) {
    ...
    private final void pushStack(Namespace namespace, Namespace[] namespaceArr, List<Namespace> list) {
        ...
        if (i >= namespaceArr2.length) {
            Namespace[][] namespaceArr3 = (Namespace[][]) ArrayCopy.copyOf((E[]) namespaceArr2, namespaceArr2.length * 2);
            this.scope = namespaceArr3;
            this.added = (Namespace[][]) ArrayCopy.copyOf((E[]) this.added, namespaceArr3.length);
        }
        if (list.isEmpty()) {
