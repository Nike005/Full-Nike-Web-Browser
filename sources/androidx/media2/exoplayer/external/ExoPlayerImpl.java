...
final class ExoPlayerImpl extends BasePlayer implements ExoPlayer {
    ...
    public void setRepeatMode(int i) {
        if (this.repeatMode != i) {
            ...
            notifyListeners((BasePlayer.ListenerInvocation) new ExoPlayerImpl$$Lambda$1(i));
        }
    }
    ...
    public void setShuffleModeEnabled(boolean z) {
        if (this.shuffleModeEnabled != z) {
            ...
            notifyListeners((BasePlayer.ListenerInvocation) new ExoPlayerImpl$$Lambda$2(z));
        }
    }
    ...
    public void seekTo(int i, long j) {
        ...
        this.pendingOperationAcks++;
        if (isPlayingAd()) {
            Log.m6210w(TAG, "seekTo ignored because an ad is playing");
        ...
        if (timeline.isEmpty()) {
            ...
            long defaultPositionUs = j == -9223372036854775807L ? timeline.getWindow(i, this.window).getDefaultPositionUs() : C3875C.msToUs(j);
            Pair<Object, Long> periodPosition = timeline.getPeriodPosition(this.window, this.period, i, defaultPositionUs);
            this.maskingWindowPositionMs = C3875C.usToMs(defaultPositionUs);
            this.maskingPeriodIndex = timeline.getIndexOfPeriod(periodPosition.first);
        }
        this.internalPlayer.seekTo(timeline, i, C3875C.msToUs(j));
    ...
    public long getDuration() {
        ...
        return C3875C.usToMs(this.period.getAdDurationUs(mediaPeriodId.adGroupIndex, mediaPeriodId.adIndexInAdGroup));
    }
    ...
    public long getCurrentPosition() {
        ...
        if (this.playbackInfo.periodId.isAd()) {
            return C3875C.usToMs(this.playbackInfo.positionUs);
        }
    ...
    public long getBufferedPosition() {
        ...
        if (this.playbackInfo.loadingMediaPeriodId.equals(this.playbackInfo.periodId)) {
            return C3875C.usToMs(this.playbackInfo.bufferedPositionUs);
        }
        return getDuration();
    }

    public long getTotalBufferedDuration() {
        return Math.max(0, C3875C.usToMs(this.playbackInfo.totalBufferedDurationUs));
    }
    ...
    public long getContentPosition() {
        ...
        return this.period.getPositionInWindowMs() + C3875C.usToMs(this.playbackInfo.contentPositionUs);
    }
    ...
    private static final class PlaybackInfoUpdate implements Runnable {
        ...
        public void run() {
            ...
            if (this.trackSelectorResultChanged) {
                this.trackSelector.onSelectionActivated(this.playbackInfo.trackSelectorResult.f5126info);
