    ...
    public <T> Request<T> add(Request<T> request) {
        ...
        synchronized (this.mWaitingRequests) {
            ...
            if (this.mWaitingRequests.containsKey(cacheKey)) {
                ...
                if (VolleyLog.DEBUG) {
                    VolleyLog.m6231v("Request for cacheKey=%s is in flight, putting on hold.", cacheKey);
                }
            } else {
    ...
    public <T> void finish(Request<T> request) {
        ...
        if (request.shouldCache()) {
            synchronized (this.mWaitingRequests) {
                ...
                if (remove != null) {
                    if (VolleyLog.DEBUG) {
                        VolleyLog.m6231v("Releasing %d waiting requests for cacheKey=%s.", Integer.valueOf(remove.size()), cacheKey);
                    }
