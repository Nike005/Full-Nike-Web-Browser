...
final class DashMediaPeriod implements MediaPeriod, SequenceableLoader.Callback<ChunkSampleStream<DashChunkSource>>, ChunkSampleStream.ReleaseCallback<DashChunkSource> {
    ...
    public void updateManifest(DashManifest dashManifest, int i) {
        ...
        for (EventSampleStream eventSampleStream : this.eventSampleStreams) {
            ...
            while (true) {
                ...
                if (next.mo261id().equals(eventSampleStream.eventStreamId())) {
                    boolean z = true;
    ...
    private static int[][] getGroupedAdaptationSetIndices(List<AdaptationSet> list) {
        ...
        for (int i = 0; i < size; i++) {
            sparseIntArray.put(list2.get(i).f2id, i);
        }
    ...
    private static int buildPrimaryAndEmbeddedTrackGroupInfos(DrmSessionManager<?> drmSessionManager2, List<AdaptationSet> list, int[][] iArr, int i, boolean[] zArr, Format[][] formatArr, TrackGroup[] trackGroupArr, TrackGroupInfo[] trackGroupInfoArr) {
        ...
        while (i5 < i4) {
            ...
            if (i9 != -1) {
                trackGroupArr[i9] = new TrackGroup(Format.createSampleFormat(adaptationSet.f2id + ":emsg", "application/x-emsg", (String) null, -1, (DrmInitData) null));
    ...
    private static void buildManifestEventTrackGroupInfos(List<EventStream> list, TrackGroup[] trackGroupArr, TrackGroupInfo[] trackGroupInfoArr, int i) {
        int i2 = 0;
        while (i2 < list.size()) {
            trackGroupArr[i] = new TrackGroup(Format.createSampleFormat(list.get(i2).mo261id(), "application/x-emsg", (String) null, -1, (DrmInitData) null));
        ...
        for (int i : iArr) {
            ...
            for (int i2 = 0; i2 < list2.size(); i2++) {
                ...
                if ("urn:scte:dash:cc:cea-608:2015".equals(descriptor.schemeIdUri)) {
                    ...
                    if (str == null) {
                        return new Format[]{buildCea608TrackFormat(adaptationSet.f2id)};
                    }
                    ...
                    for (int i3 = 0; i3 < split.length; i3++) {
                        ...
                        formatArr[i3] = buildCea608TrackFormat(adaptationSet.f2id, matcher.group(2), Integer.parseInt(matcher.group(1)));
                    }
                    return formatArr;
                }
            }
