...
public class ScanJob extends JobService {
    ...
    public boolean onStartJob(final JobParameters jobParameters) {
        ...
        if (jobParameters.getJobId() == getImmediateScanJobId(this)) {
            String str = TAG;
            LogManager.m6050i(str, "Running immediate scan job: instance is " + this, new Object[0]);
        } else {
            String str2 = TAG;
            LogManager.m6050i(str2, "Running periodic scan job: instance is " + this, new Object[0]);
        }
        List<ScanResult> dumpBackgroundScanResultQueue = ScanJobScheduler.getInstance().dumpBackgroundScanResultQueue();
        LogManager.m6046d(TAG, "Processing %d queued scan resuilts", Integer.valueOf(dumpBackgroundScanResultQueue.size()));
        ...
        LogManager.m6046d(TAG, "Done processing queued scan resuilts", new Object[0]);
        if (this.mInitialized) {
            LogManager.m6046d(TAG, "Scanning already started.  Resetting for current parameters", new Object[0]);
            z = restartScanning();
        } else {
            z = startScanning();
        }
        ...
        if (z) {
            String str3 = TAG;
            LogManager.m6050i(str3, "Scan job running for " + this.mScanState.getScanJobRuntimeMillis() + " millis", new Object[0]);
            this.mStopHandler.postDelayed(new Runnable() {
                public void run() {
                    String access$000 = ScanJob.TAG;
                    LogManager.m6050i(access$000, "Scan job runtime expired: " + ScanJob.this, new Object[0]);
            ...
            LogManager.m6050i(TAG, "Scanning not started so Scan job is complete.", new Object[0]);
    ...
    public void scheduleNextScan() {
        if (!this.mScanState.getBackgroundMode().booleanValue()) {
            LogManager.m6046d(TAG, "In foreground mode, schedule next scan", new Object[0]);
    ...
    private void startPassiveScanIfNeeded() {
        LogManager.m6046d(TAG, "Checking to see if we need to start a passive scan", new Object[0]);
        boolean z = false;
        ...
        if (z) {
            LogManager.m6050i(TAG, "We are inside a beacon region.  We will not scan between cycles.", new Object[0]);
        } else if (Build.VERSION.SDK_INT >= 26) {
            this.mScanHelper.startAndroidOBackgroundScan(this.mScanState.getBeaconParsers());
        } else {
            LogManager.m6046d(TAG, "This is not Android O.  No scanning between cycles when using ScanJob", new Object[0]);
        }
    }
    ...
    public boolean onStopJob(JobParameters jobParameters) {
        if (jobParameters.getJobId() == getPeriodicScanJobId(this)) {
            String str = TAG;
            LogManager.m6050i(str, "onStopJob called for periodic scan " + this, new Object[0]);
        } else {
            String str2 = TAG;
            LogManager.m6050i(str2, "onStopJob called for immediate scan " + this, new Object[0]);
        }
        this.mStopHandler.removeCallbacksAndMessages((Object) null);
        stopScanning();
        startPassiveScanIfNeeded();
    ...
    public void stopScanning() {
        ...
        LogManager.m6046d(TAG, "Scanning stopped", new Object[0]);
    }
    ...
    private boolean restartScanning() {
        ...
        if (longValue <= 0) {
            LogManager.m6054w(TAG, "Starting scan with scan period of zero.  Exiting ScanJob.", new Object[0]);
            this.mScanHelper.getCycledScanner().stop();
            return false;
    ...
    private boolean startScanning() {
        ...
        if (instanceForApplication.isMainProcess()) {
            LogManager.m6050i(TAG, "scanJob version %s is starting up on the main process", BuildConfig.VERSION_NAME);
        } else {
            LogManager.m6050i(TAG, "beaconScanJob library version %s is starting up on a separate process", BuildConfig.VERSION_NAME);
            ProcessUtils processUtils = new ProcessUtils(this);
            String str = TAG;
            LogManager.m6050i(str, "beaconScanJob PID is " + processUtils.getPid() + " with process name " + processUtils.getProcessName(), new Object[0]);
        }
    ...
    public static int getImmediateScanJobId(Context context) {
        ...
        String str = TAG;
        LogManager.m6050i(str, "Using ImmediateScanJobId from static override: " + sOverrideImmediateScanJobId, new Object[0]);
    ...
    public static int getPeriodicScanJobId(Context context) {
        ...
        String str = TAG;
        LogManager.m6050i(str, "Using PeriodicScanJobId from static override: " + sOverridePeriodicScanJobId, new Object[0]);
    ...
    private static int getJobIdFromManifest(Context context, String str) {
        ...
        String str2 = TAG;
        LogManager.m6050i(str2, "Using " + str + " from manifest: " + i, new Object[0]);
        return i;
    }
}
