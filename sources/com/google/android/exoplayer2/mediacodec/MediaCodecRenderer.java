...
public abstract class MediaCodecRenderer extends BaseRenderer {
            ...
            while (this.codec == null) {
                ...
                if (shouldInitCodec(peekFirst)) {
                    try {
                        ...
                        Log.m10w(TAG, "Failed to initialize decoder: " + peekFirst, e2);
    ...
    private List<MediaCodecInfo> getAvailableCodecInfos(boolean z) throws MediaCodecUtil.DecoderQueryException {
        ...
        if (decoderInfos.isEmpty() && z) {
            ...
            if (!decoderInfos.isEmpty()) {
                Log.m9w(TAG, "Drm session requires secure decoder for " + this.inputFormat.sampleMimeType + ", but no secure decoder available. Trying to proceed with " + decoderInfos + ".");
            }
        }
        return decoderInfos;
    }
    ...
    private void updateDrmSessionOrReinitializeCodecV23() throws ExoPlaybackException {
        ...
        if (mediaCrypto2 == null) {
            reinitializeCodec();
        } else if (C5211C.PLAYREADY_UUID.equals(mediaCrypto2.uuid)) {
            reinitializeCodec();
