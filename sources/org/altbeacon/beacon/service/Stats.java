...
public class Stats {
    ...
    private void logSample(Sample sample, boolean z) {
        if (z) {
            LogManager.m6046d(TAG, "sample start time, sample stop time, first detection time, last detection time, max millis between detections, detection count", new Object[0]);
        }
        LogManager.m6046d(TAG, "%s, %s, %s, %s, %s, %s", formattedDate(sample.sampleStartTime), formattedDate(sample.sampleStopTime), formattedDate(sample.firstDetectionTime), formattedDate(sample.lastDetectionTime), Long.valueOf(sample.maxMillisBetweenDetections), Long.valueOf(sample.detectionCount));
    }
    ...
    private void logSamples() {
        boolean z = true;
        LogManager.m6046d(TAG, "--- Stats for %s samples", Integer.valueOf(this.mSamples.size()));
