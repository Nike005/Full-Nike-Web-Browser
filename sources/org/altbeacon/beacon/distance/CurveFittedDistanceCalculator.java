...
public class CurveFittedDistanceCalculator implements DistanceCalculator {
    ...
    public double calculateDistance(int i, double d) {
        ...
        LogManager.m6046d(TAG, "calculating distance based on mRssi of %s and txPower of %s", Double.valueOf(d), Integer.valueOf(i));
        double d3 = (double) i;
        Double.isNaN(d3);
        double d4 = (d * 1.0d) / d3;
        if (d4 < 1.0d) {
        ...
        LogManager.m6046d(TAG, "avg mRssi: %s distance: %s", Double.valueOf(d), Double.valueOf(d2));
        return d2;
    }
}
