...
final class WavHeaderReader {
    ...
    public static WavHeader peek(ExtractorInput extractorInput) throws IOException, InterruptedException {
        ...
        if (ChunkHeader.peek(extractorInput, parsableByteArray).f5062id != 1380533830) {
            return null;
        }
        ...
        if (readInt != 1463899717) {
            Log.m5e(TAG, "Unsupported RIFF format: " + readInt);
            return null;
        }
        ChunkHeader peek = ChunkHeader.peek(extractorInput, parsableByteArray);
        while (peek.f5062id != 1718449184) {
        ...
        if (readLittleEndianUnsignedShort3 == i) {
            ...
            if (encodingForType == 0) {
                Log.m5e(TAG, "Unsupported WAV format: " + readLittleEndianUnsignedShort4 + " bit/sample, type " + readLittleEndianUnsignedShort);
                return null;
            }
    ...
    public static void skipToData(ExtractorInput extractorInput, WavHeader wavHeader) throws IOException, InterruptedException {
        ...
        ChunkHeader peek = ChunkHeader.peek(extractorInput, parsableByteArray);
        while (peek.f5062id != 1684108385) {
            if (!(peek.f5062id == 1380533830 || peek.f5062id == 1718449184)) {
                Log.m9w(TAG, "Ignoring unknown WAV chunk: " + peek.f5062id);
            }
            long j = peek.size + 8;
            if (peek.f5062id == 1380533830) {
                j = 12;
            }
            if (j <= 2147483647L) {
                ...
                throw new ParserException("Chunk is too large (~2GB+) to skip; id: " + peek.f5062id);
            }
        }
        ...
        if (length != -1 && j2 > length) {
            Log.m9w(TAG, "Data exceeds input length: " + j2 + ", " + length);
            j2 = length;
        }
