...
public class Banner3D extends BannerBase implements BannerInterface, AdEventListener {
    ...
    public Banner3D(Context context, boolean z, AdPreferences adPreferences2) {
            ...
            public void run() {
                if (Banner3D.this.loaded && Banner3D.this.faces.size() != 0) {
                    ...
                    if (Banner3D.this.rotationEnabled) {
                        ...
                        banner3D.rotate((float) (banner3D.getBannerOptions().mo13879b() * (!Banner3D.this.firstRotationFinished ? Banner3D.this.options.mo13894p() : 1)));
                    }
                    if (Banner3D.this.rotation <= ((float) (90 - Banner3D.this.getBannerOptions().mo13879b())) || Banner3D.this.rotation >= ((float) (Banner3D.this.getBannerOptions().mo13879b() + 90)) || Banner3D.this.firstRotation) {
                        Banner3D banner3D2 = Banner3D.this;
                        banner3D2.postDelayed(this, (long) banner3D2.getBannerOptions().mo13877a());
                        Banner3D.this.rotating = true;
                    } else {
                        if (Banner3D.this.attachedToWindow) {
                            Banner3D banner3D3 = Banner3D.this;
                            banner3D3.postDelayed(this, (long) banner3D3.getBannerOptions().mo13880c());
                        }
        ...
        try {
            ...
            C1132f.m1527a(context, C1130d.EXCEPTION, "Banner.init - unexpected error occurd", e.getMessage(), "");
        }
    }
    ...
    private void addAdInformationLayout() {
        ...
        new C1084b(getContext(), C1084b.C1092b.SMALL, AdPreferences.Placement.INAPP_BANNER, this.overrides).mo14713a(relativeLayout);
    }
    ...
    public void reload() {
        ...
        cleanFaces();
        this.faces = new ArrayList();
        this.ads = new C0888a(getContext(), getOffset());
        if (this.adPreferences == null) {
            this.adPreferences = new AdPreferences();
        }
    ...
    public void loadBanners(List<AdDetails> list, boolean z) {
        this.adsItems = list;
        if (list != null) {
            C0902d dVar = new C0902d();
            if (setBannerSize(dVar)) {
                setMinimumWidth(C1060h.m1162a(getContext(), this.options.mo13881d()));
                setMinimumHeight(C1060h.m1162a(getContext(), this.options.mo13882e()));
                if (getLayoutParams() != null && getLayoutParams().width == -1) {
                    setMinimumWidth(C1060h.m1162a(getContext(), dVar.mo13975a()));
                }
                if (getLayoutParams() != null && getLayoutParams().height == -1) {
                    setMinimumHeight(C1060h.m1162a(getContext(), dVar.mo13978b()));
                }
            ...
            C1270g.m2078a(TAG, 6, "Banner3DError in banner screen size");
    ...
    public void makeImpression(Banner3DFace banner3DFace) {
        C1155i a = banner3DFace.mo13930a(getContext());
        if (a != null) {
            C1270g.m2078a(TAG, 3, "Banner3D Scheduling visibility check");
    ...
    private void initFacesViews() {
        for (Banner3DFace a : this.faces) {
            a.mo13932a(getContext(), getBannerOptions(), this);
        }
    }
    ...
    private void createFaces(List<AdDetails> list) {
        ...
        for (AdDetails banner3DFace : list) {
            this.faces.add(new Banner3DFace(getContext(), this, banner3DFace, getBannerOptions(), new C1117b(getAdTag())));
        }
        this.currentBannerIndex = 0;
    }

    private boolean setBannerSize(C0902d dVar) {
        return Banner3DSize.m608a(getContext(), getParent(), getBannerOptions(), this, dVar);
    }

    private Bitmap getCurrentBitmap() {
        return this.faces.get(getCurrentBannerIndex()).mo13934b();
    }

    private Bitmap getPreviousBitmap() {
        return this.faces.get(((getCurrentBannerIndex() - 1) + this.faces.size()) % this.faces.size()).mo13934b();
    }
    ...
    private void drawFrame(Canvas canvas) {
        try {
            ...
            int faceStartLeft = getFaceStartLeft();
            int faceStartTop = getFaceStartTop();
            float l = this.options.mo13890l() + (((float) Math.pow((double) (Math.abs(this.rotation - 45.0f) / 45.0f), (double) this.options.mo13891m())) * (1.0f - this.options.mo13890l()));
            if (!this.firstRotationFinished) {
                l = this.options.mo13890l();
            }
            float f = l;
            ...
            if (currentBitmap != null && previousBitmap != null) {
                if (this.rotation < 45.0f) {
                    if (this.rotation > 3.0f) {
                        ...
                        drawFace(canvas2, bitmap, i, i2, faceWidth / 2, faceHeight / 2, f, (this.rotation - 90.0f) * ((float) this.options.mo13892n().getRotationMultiplier()));
                    }
                    Canvas canvas3 = canvas;
                    Bitmap bitmap2 = previousBitmap;
                    int i3 = faceStartTop;
                    int i4 = faceStartLeft;
                    drawFace(canvas3, bitmap2, i3, i4, faceWidth / 2, faceHeight / 2, f, this.rotation * ((float) this.options.mo13892n().getRotationMultiplier()));
                    return;
                }
                if (this.rotation < 87.0f) {
                    ...
                    drawFace(canvas4, bitmap3, i5, i6, faceWidth / 2, faceHeight / 2, f, this.rotation * ((float) this.options.mo13892n().getRotationMultiplier()));
                }
                Canvas canvas5 = canvas;
                Bitmap bitmap4 = currentBitmap;
                int i7 = faceStartTop;
                int i8 = faceStartLeft;
                drawFace(canvas5, bitmap4, i7, i8, faceWidth / 2, faceHeight / 2, f, (this.rotation - 90.0f) * ((float) this.options.mo13892n().getRotationMultiplier()));
                if (!this.firstRotation) {
                    this.firstRotationFinished = true;
                }
            }
        } catch (Exception unused) {
            C1270g.m2078a(TAG, 6, "Exception onDraw Banner3D");
        }
    }
    ...
    private int getFaceHeight() {
        return (int) (((float) C1060h.m1162a(getContext(), this.options.mo13882e())) * this.options.mo13889k());
    }

    private int getFaceWidth() {
        return (int) (((float) C1060h.m1162a(getContext(), this.options.mo13881d())) * this.options.mo13888j());
    }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (action == 0) {
            ...
            }, C1098b.m1303a().mo14747A());
            this.faces.get(getCurrentBannerIndex()).mo13935b(getContext());
            BannerListener bannerListener = this.listener;
    ...
    public void onReceiveAd(C1040Ad ad) {
        ...
        loadBanners(((C1148h) ad).mo14937d(), true);
    }
    ...
    public void onAttachedToWindow() {
        ...
        if (bannerOptions == null || !bannerOptions.mo13893o()) {
    ...
    private static class SavedState extends View.BaseSavedState {
        ...
        private SavedState(Parcel parcel) {
            ...
            if (parcel.readInt() == 1) {
                ...
                this.overrides = (C1093c) parcel.readSerializable();
                this.options = (BannerOptions) parcel.readSerializable();
                this.adRulesResult = (C1073f) parcel.readSerializable();
                return;
            }
    ...
    public int getRefreshRate() {
        return C0901c.m627a().mo13973b().mo13886h();
    }

    /* access modifiers changed from: protected */
    public int getOffset() {
        C0888a aVar = this.ads;
        if (aVar == null) {
            return 0;
        }
        return aVar.mo13946a();
    }
