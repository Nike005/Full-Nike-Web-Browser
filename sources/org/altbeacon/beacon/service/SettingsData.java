...
public class SettingsData implements Serializable {
    ...
    public void apply(BeaconService beaconService) {
        LogManager.m6046d(TAG, "Applying settings changes to scanner in other process", new Object[0]);
        ...
        if (beaconParsers.size() == this.mBeaconParsers.size()) {
            ...
            while (true) {
                if (i >= beaconParsers.size()) {
                    ...
                    String str = TAG;
                    LogManager.m6046d(str, "Beacon parsers have changed to: " + this.mBeaconParsers.get(i).getLayout(), new Object[0]);
                    break;
                } else {
                    i++;
                }
            ...
            LogManager.m6046d(TAG, "Beacon parsers have been added or removed.", new Object[0]);
        }
        if (z) {
            LogManager.m6046d(TAG, "Updating beacon parsers", new Object[0]);
            instanceForApplication.getBeaconParsers().clear();
            instanceForApplication.getBeaconParsers().addAll(this.mBeaconParsers);
            beaconService.reloadParsers();
        } else {
            LogManager.m6046d(TAG, "Beacon parsers unchanged.", new Object[0]);
        }
