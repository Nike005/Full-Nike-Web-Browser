    ...
    private void onMoovContainerAtomRead(Atom.ContainerAtom containerAtom) throws ParserException {
        ...
        while (i5 < size2) {
            ...
            if (containerAtom3.type == 1953653099) {
                ...
                if (modifyTrack != null) {
                    sparseArray2.put(modifyTrack.f4083id, modifyTrack);
                }
            } else {
                i = i5;
                i2 = size2;
        ...
        if (this.trackBundles.size() == 0) {
            while (i3 < size3) {
                ...
                trackBundle.init(track, getDefaultSampleValues(sparseArray, track.f4083id));
                this.trackBundles.put(track.f4083id, trackBundle);
        ...
        while (i3 < size3) {
            ...
            this.trackBundles.get(track2.f4083id).init(track2, getDefaultSampleValues(sparseArray, track2.f4083id));
            i3++;
        }
    }
    ...
    private boolean readSample(ExtractorInput extractorInput) throws IOException, InterruptedException {
        ...
        if (this.parserState == 3) {
            if (this.currentTrackBundle == null) {
                ...
                if (position2 < 0) {
                    Log.m6210w(TAG, "Ignoring negative offset to sample data.");
                    position2 = 0;
                }
    ...
    private static DrmInitData getDrmInitDataFromAtoms(List<Atom.LeafAtom> list) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (leafAtom.type == 1886614376) {
                ...
                if (parseUuid == null) {
                    Log.m6210w(TAG, "Skipped pssh atom (failed to extract uuid)");
                } else {
