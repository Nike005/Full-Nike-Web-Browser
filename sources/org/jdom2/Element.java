...
public class Element extends Content implements Parent {
    ...
    public Element setNamespace(Namespace namespace2) {
        ...
        if (this.additionalNamespaces == null || (checkNamespaceCollision = Verifier.checkNamespaceCollision(namespace2, (List<?>) getAdditionalNamespaces())) == null) {
            if (hasAttributes()) {
    ...
    public Namespace getNamespace(String str) {
        ...
        if (this.additionalNamespaces != null) {
            for (int i = 0; i < this.additionalNamespaces.size(); i++) {
                Namespace namespace2 = this.additionalNamespaces.get(i);
    ...
    public String getAttributeValue(String str, Namespace namespace2, String str2) {
        Attribute attribute;
        if (this.attributes == null || (attribute = getAttributeList().get(str, namespace2)) == null) {
            return str2;
        }
    ...
    public List<Namespace> getNamespacesInScope() {
        ...
        if (this.additionalNamespaces != null) {
            for (Namespace next : getAdditionalNamespaces()) {
                if (!treeMap.containsKey(next.getPrefix())) {
                    treeMap.put(next.getPrefix(), next);
                }
            }
        }
        if (this.attributes != null) {
            for (Attribute namespace2 : getAttributes()) {
        ...
        if (parentElement != null) {
            for (Namespace next2 : parentElement.getNamespacesInScope()) {
                if (!treeMap.containsKey(next2.getPrefix())) {
                    treeMap.put(next2.getPrefix(), next2);
                }
            }
        }
    ...
    public List<Namespace> getNamespacesInherited() {
        ...
        for (Namespace next : getParentElement().getNamespacesInScope()) {
            hashMap.put(next.getPrefix(), next);
        }
        ArrayList arrayList2 = new ArrayList();
        for (Namespace next2 : getNamespacesInScope()) {
            if (next2 == hashMap.get(next2.getPrefix())) {
    ...
    public List<Namespace> getNamespacesIntroduced() {
        ...
        ArrayList arrayList2 = new ArrayList();
        for (Namespace next2 : getNamespacesInScope()) {
            if (!hashMap.containsKey(next2.getPrefix()) || next2 != hashMap.get(next2.getPrefix())) {
                arrayList2.add(next2);
            }
        }
        return Collections.unmodifiableList(arrayList2);
