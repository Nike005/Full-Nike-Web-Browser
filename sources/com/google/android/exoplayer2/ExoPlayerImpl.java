...
final class ExoPlayerImpl extends BasePlayer implements ExoPlayer {
    ...
    public ExoPlayerImpl(Renderer[] rendererArr, TrackSelector trackSelector2, LoadControl loadControl, BandwidthMeter bandwidthMeter, Clock clock, Looper looper) {
        ...
        Log.m7i(TAG, "Init " + Integer.toHexString(System.identityHashCode(this)) + " [" + ExoPlayerLibraryInfo.VERSION_SLASHY + "] [" + Util.DEVICE_DEBUG_INFO + "]");
    ...
    public void seekTo(int i, long j) {
        ...
        this.pendingOperationAcks++;
        if (isPlayingAd()) {
            Log.m9w(TAG, "seekTo ignored because an ad is playing");
        ...
        if (timeline.isEmpty()) {
            ...
            long defaultPositionUs = j == -9223372036854775807L ? timeline.getWindow(i, this.window).getDefaultPositionUs() : C5211C.msToUs(j);
            Pair<Object, Long> periodPosition = timeline.getPeriodPosition(this.window, this.period, i, defaultPositionUs);
            this.maskingWindowPositionMs = C5211C.usToMs(defaultPositionUs);
            this.maskingPeriodIndex = timeline.getIndexOfPeriod(periodPosition.first);
        }
        this.internalPlayer.seekTo(timeline, i, C5211C.msToUs(j));
    ...
    public void release() {
        Log.m7i(TAG, "Release " + Integer.toHexString(System.identityHashCode(this)) + " [" + ExoPlayerLibraryInfo.VERSION_SLASHY + "] [" + Util.DEVICE_DEBUG_INFO + "] [" + ExoPlayerLibraryInfo.registeredModules() + "]");
        this.mediaSource = null;
    ...
    public long getDuration() {
        ...
        return C5211C.usToMs(this.period.getAdDurationUs(mediaPeriodId.adGroupIndex, mediaPeriodId.adIndexInAdGroup));
    }
    ...
    public long getCurrentPosition() {
        ...
        if (this.playbackInfo.periodId.isAd()) {
            return C5211C.usToMs(this.playbackInfo.positionUs);
        }
    ...
    public long getBufferedPosition() {
        ...
        if (this.playbackInfo.loadingMediaPeriodId.equals(this.playbackInfo.periodId)) {
            return C5211C.usToMs(this.playbackInfo.bufferedPositionUs);
        }
        return getDuration();
    }

    public long getTotalBufferedDuration() {
        return C5211C.usToMs(this.playbackInfo.totalBufferedDurationUs);
    }
    ...
    public long getContentPosition() {
        ...
        return this.period.getPositionInWindowMs() + C5211C.usToMs(this.playbackInfo.contentPositionUs);
    }
    ...
    private static final class PlaybackInfoUpdate implements Runnable {
        ...
        public void run() {
            ...
            if (this.trackSelectorResultChanged) {
                this.trackSelector.onSelectionActivated(this.playbackInfo.trackSelectorResult.f5083info);
                ExoPlayerImpl.invokeAll(this.listenerSnapshot, new BasePlayer.ListenerInvocation() {
                    public final void invokeListener(Player.EventListener eventListener) {
