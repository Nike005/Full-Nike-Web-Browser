...
public class PermissionsActivity extends Activity {
    ...
    private void requestPermission() {
        if (Build.VERSION.SDK_INT < 23) {
            finish();
            overridePendingTransition(C0770R.C0771anim.onesignal_fade_in, C0770R.C0771anim.onesignal_fade_out);
        } else if (!waiting) {
            waiting = true;
    ...
    private void showLocationPermissionSettings() {
        new AlertDialog.Builder(ActivityLifecycleHandler.curActivity).setTitle(C0770R.string.location_not_available_title).setMessage(C0770R.string.location_not_available_open_settings_message).setPositiveButton(C0770R.string.location_not_available_open_settings_option, new DialogInterface.OnClickListener() {
    ...
    static void startPrompt(boolean z) {
        if (!waiting && !answered) {
            ...
            C07664 r1 = new ActivityLifecycleHandler.ActivityAvailableListener() {
                public void available(Activity activity) {
                    if (!activity.getClass().equals(PermissionsActivity.class)) {
                        Intent intent = new Intent(activity, PermissionsActivity.class);
                        intent.setFlags(131072);
                        activity.startActivity(intent);
                        activity.overridePendingTransition(C0770R.C0771anim.onesignal_fade_in, C0770R.C0771anim.onesignal_fade_out);
                    }
                }
            };
