...
public final class zzvw extends zzaop<zzuu> {
    ...
    private final void zzmc() {
        synchronized (this.mLock) {
            ...
            if (!this.zzbqt || this.zzbqu != 0) {
                zzakb.m43v("There are still references to the engine. Not destroying.");
            } else {
                zzakb.m43v("No reference is left (including root). Cleaning up engine.");
    ...
    public final void zzma() {
        synchronized (this.mLock) {
            Preconditions.checkState(this.zzbqu > 0);
            zzakb.m43v("Releasing 1 reference for JS Engine");
            this.zzbqu--;
            zzmc();
        }
    }
    ...
    public final void zzmb() {
        synchronized (this.mLock) {
            Preconditions.checkState(this.zzbqu >= 0);
            zzakb.m43v("Releasing root reference. JS Engine will be destroyed once other references are released.");
            this.zzbqt = true;
            zzmc();
        }
    }
