...
public class ScanJobScheduler {
    ...
    private void applySettingsToScheduledJob(Context context, BeaconManager beaconManager, ScanState scanState) {
        ...
        String str = TAG;
        LogManager.m6046d(str, "Applying scan job settings with background mode " + scanState.getBackgroundMode(), new Object[0]);
        schedule(context, scanState, false);
    }

    public void applySettingsToScheduledJob(Context context, BeaconManager beaconManager) {
        LogManager.m6046d(TAG, "Applying settings to ScanJob", new Object[0]);
    ...
    public void scheduleAfterBackgroundWakeup(Context context, List<ScanResult> list) {
        ...
        synchronized (this) {
            if (System.currentTimeMillis() - this.mScanJobScheduleTime.longValue() > 10000) {
                String str = TAG;
                LogManager.m6046d(str, "scheduling an immediate scan job because last did " + (System.currentTimeMillis() - this.mScanJobScheduleTime.longValue()) + "seconds ago.", new Object[0]);
                this.mScanJobScheduleTime = Long.valueOf(System.currentTimeMillis());
                schedule(context, ScanState.restore(context), true);
                return;
            }
            LogManager.m6046d(TAG, "Not scheduling an immediate scan job because we just did recently.", new Object[0]);
        }
    }
    ...
    private void schedule(Context context, ScanState scanState, boolean z) {
        ...
        long j = 50;
        if (z) {
            LogManager.m6046d(TAG, "We just woke up in the background based on a new scan result.  Start scan job immediately.", new Object[0]);
            j = 0;
        } else {
        ...
        if (!z && scanState.getBackgroundMode().booleanValue()) {
            LogManager.m6046d(TAG, "Not scheduling an immediate scan because we are in background mode.   Cancelling existing immediate scan.", new Object[0]);
            jobScheduler.cancel(ScanJob.getImmediateScanJobId(context));
        } else if (j < ((long) (scanState.getScanJobIntervalMillis() - 50))) {
            LogManager.m6046d(TAG, "Scheduling immediate ScanJob to run in " + j + " millis", new Object[0]);
            ...
            if (schedule < 0) {
                LogManager.m6048e(TAG, "Failed to schedule scan job.  Beacons will not be detected. Error: " + schedule, new Object[0]);
            }
        } else {
            LogManager.m6046d(TAG, "Not scheduling immediate scan, assuming periodic is about to run", new Object[0]);
        }
        ...
        LogManager.m6046d(TAG, "Scheduling ScanJob " + build + " to run every " + scanState.getScanJobIntervalMillis() + " millis", new Object[0]);
        int schedule2 = jobScheduler.schedule(build);
        if (schedule2 < 0) {
            LogManager.m6048e(TAG, "Failed to schedule scan job.  Beacons will not be detected. Error: " + schedule2, new Object[0]);
        }
    }
}
