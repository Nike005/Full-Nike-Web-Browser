...
public class Callback implements Serializable {
    ...
    public boolean call(Context context, String str, Bundle bundle) {
        if (BeaconManager.getInstanceForApplication(context).getScheduledScanJobsEnabled()) {
            ...
            LogManager.m6046d(TAG, "attempting callback via local broadcast intent: %s", str2);
        ...
        LogManager.m6046d(TAG, "attempting callback via global broadcast intent: %s", intent2.getComponent());
        try {
            context.startService(intent2);
            return true;
        } catch (Exception e) {
            LogManager.m6048e(TAG, "Failed attempting to start service: " + intent2.getComponent().flattenToString(), e);
            return false;
        }
    }
