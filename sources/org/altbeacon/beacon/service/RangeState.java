    ...
    public void addBeacon(Beacon beacon) {
        if (this.mRangedBeacons.containsKey(beacon)) {
            ...
            if (LogManager.isVerboseLoggingEnabled()) {
                LogManager.m6046d(TAG, "adding %s to existing range for: %s", beacon, rangedBeacon);
            }
            rangedBeacon.updateBeacon(beacon);
            return;
        }
        if (LogManager.isVerboseLoggingEnabled()) {
            LogManager.m6046d(TAG, "adding %s to new rangedBeacon", beacon);
        }
        ...
        synchronized (this.mRangedBeacons) {
            for (Beacon next : this.mRangedBeacons.keySet()) {
                ...
                if (!rangedBeacon.noMeasurementsAvailable()) {
                    ...
                    LogManager.m6046d(TAG, "Dumping beacon from RangeState because it has no recent measurements.", new Object[0]);
                }
            }
