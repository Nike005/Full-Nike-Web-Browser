...
public class HtmlTreeBuilder extends TreeBuilder {
    ...
    private static final String[] TagSearchSpecial = {"address", "applet", "area", "article", "aside", "base", "basefont", "bgsound", "blockquote", "body", "br", "button", "caption", "center", "col", "colgroup", "command", "dd", "details", "dir", "div", "dl", "dt", "embed", "fieldset", "figcaption", "figure", "footer", "form", "frame", "frameset", "h1", "h2", "h3", "h4", "h5", "h6", "head", "header", "hgroup", "hr", AdType.HTML, "iframe", "img", "input", "isindex", "li", "link", "listing", "marquee", "menu", "meta", "nav", "noembed", "noframes", "noscript", HomepageView.HomePageViewPagerFragment.ARG_OBJECT, "ol", "p", "param", "plaintext", "pre", "script", "section", "select", "style", "summary", "table", "tbody", "td", "textarea", "tfoot", "th", "thead", "title", "tr", "ul", "wbr", "xmp"};
    private static final String[] TagSearchTableScope = {AdType.HTML, "table"};
    public static final String[] TagsSearchInScope = {"applet", "caption", AdType.HTML, "table", "td", "th", "marquee", HomepageView.HomePageViewPagerFragment.ARG_OBJECT};
    ...
    public List<Node> parseFragment(String str, Element element, String str2, ParseErrorList parseErrorList, ParseSettings parseSettings) {
        ...
        if (element != null) {
            ...
            if (StringUtil.m6103in(tagName, "title", "textarea")) {
                this.tokeniser.transition(TokeniserState.Rcdata);
            } else if (StringUtil.m6103in(tagName, "iframe", "noembed", "noframes", "style", "xmp")) {
            ...
            element2 = new Element(Tag.valueOf(AdType.HTML, parseSettings), str2);
    ...
    public void popStackToClose(String... strArr) {
        ...
        while (size >= 0) {
            this.stack.remove(size);
            if (!StringUtil.m6103in(((Element) this.stack.get(size)).nodeName(), strArr)) {
                size--;
            } else {
                return;
            }
    ...
    private void clearStackToContext(String... strArr) {
        ...
        while (size >= 0) {
            Element element = (Element) this.stack.get(size);
            if (!StringUtil.m6103in(element.nodeName(), strArr) && !element.nodeName().equals(AdType.HTML)) {
                this.stack.remove(size);
                size--;
            } else {
                return;
    ...
    private boolean inSpecificScope(String[] strArr, String[] strArr2, String[] strArr3) {
        for (int size = this.stack.size() - 1; size >= 0; size--) {
            ...
            if (StringUtil.m6103in(nodeName, strArr)) {
                return true;
            }
            if (StringUtil.m6103in(nodeName, strArr2)) {
                return false;
            }
            if (strArr3 != null && StringUtil.m6103in(nodeName, strArr3)) {
                return false;
            }
        }
    ...
    public boolean inSelectScope(String str) {
        for (int size = this.stack.size() - 1; size >= 0; size--) {
            ...
            if (!StringUtil.m6103in(nodeName, TagSearchSelectScope)) {
                return false;
            }
        }
        Validate.fail("Should not be reachable");
    ...
    public void generateImpliedEndTags(String str) {
        while (str != null && !currentElement().nodeName().equals(str) && StringUtil.m6103in(currentElement().nodeName(), TagSearchEndTags)) {
            pop();
        }
    }
    ...
    public boolean isSpecial(Element element) {
        return StringUtil.m6103in(element.nodeName(), TagSearchSpecial);
    }
