...
public class RangedBeacon implements Serializable {
    ...
    public void commitMeasurements() {
        if (!getFilter().noMeasurementsAvailable()) {
            ...
            LogManager.m6046d(TAG, "calculated new runningAverageRssi: %s", Double.valueOf(calculateRssi));
        } else {
            LogManager.m6046d(TAG, "No measurements available to calculate running average", new Object[0]);
        }
    ...
    private RssiFilter getFilter() {
        if (this.mFilter == null) {
            try {
                ...
                LogManager.m6048e(TAG, "Could not construct RssiFilterImplClass %s", BeaconManager.getRssiFilterImplClass().getName());
            }
        }
        return this.mFilter;
    }
