...
class JDOMCoreNavigator extends DefaultNavigator {
    ...
    public final Object getDocument(String str) throws FunctionCallException {
        try {
            ...
            throw new FunctionCallException("Failed to parse " + str, e);
        } catch (IOException e2) {
            throw new FunctionCallException("Failed to access " + str, e2);
        }
    }
    ...
    private final void recurseElementText(Element element, StringBuilder sb) {
        for (Content next : element.getContent()) {
            if (next instanceof Element) {
                recurseElementText((Element) next, sb);
    ...
    public final Iterator<?> getParentAxisIterator(Object obj) throws UnsupportedAxisException {
        Object obj2;
        if (obj instanceof Content) {
            obj2 = ((Content) obj).getParent();
        } else if (obj instanceof NamespaceContainer) {
            obj2 = ((NamespaceContainer) obj).getParentElement();
        } else {
            obj2 = obj instanceof Attribute ? ((Attribute) obj).getParent() : null;
        }
        if (obj2 != null) {
