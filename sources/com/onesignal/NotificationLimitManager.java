...
class NotificationLimitManager {
    ...
    static void clearOldestOverLimitFallback(Context context, int i) {
        Cursor cursor = null;
        try {
            cursor = OneSignalDbHelper.getInstance(context).getSQLiteDatabaseWithRetries().query(OneSignalDbContract.NotificationTable.TABLE_NAME, new String[]{OneSignalDbContract.NotificationTable.COLUMN_NAME_ANDROID_NOTIFICATION_ID}, OneSignalDbHelper.recentUninteractedWithNotificationsWhere().toString(), (String[]) null, (String) null, (String) null, "_id", getMaxNumberOfNotificationsString() + i);
