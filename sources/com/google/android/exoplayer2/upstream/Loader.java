    ...
    private final class LoadTask<T extends Loadable> extends Handler implements Runnable {
        ...
        public void run() {
            try {
                ...
                Log.m6e(TAG, "Unexpected exception loading stream", e2);
                if (!this.released) {
                    obtainMessage(3, new UnexpectedLoaderException(e2)).sendToTarget();
                }
            } catch (OutOfMemoryError e3) {
                Log.m6e(TAG, "OutOfMemory error loading stream", e3);
                if (!this.released) {
                    obtainMessage(3, new UnexpectedLoaderException(e3)).sendToTarget();
                }
            } catch (Error e4) {
                Log.m6e(TAG, "Unexpected error loading stream", e4);
                if (!this.released) {
                ...
                if (message.what == 0) {
                    ...
                    if (i == 1) {
                        ...
                        try {
                            ...
                            Log.m6e(TAG, "Unexpected exception handling load completed", e);
