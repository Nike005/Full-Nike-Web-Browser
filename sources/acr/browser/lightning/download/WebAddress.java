...
class WebAddress {
    ...
    private static final Pattern sAddressPattern = Pattern.compile("(?:(http|https|file)://)?(?:([-A-Za-z0-9$_.+!*'(),;?&=]+(?::[-A-Za-z0-9$_.+!*'(),;?&=]+)?)@)?([a-zA-Z0-9 -퟿豈-﷏ﷰ-￯%_-][a-zA-Z0-9 -퟿豈-﷏ﷰ-￯%_\\.-]*|\\[[0-9a-fA-F:\\.]+\\])?(?::([0-9]*))?(/?[^#]*)?.*", 2);
    ...
    public WebAddress(String str) throws IllegalArgumentException {
        ...
        if (str != null) {
            ...
            if (matcher.matches()) {
                ...
                if (this.mPort == 443 && (str2 = this.mScheme) != null && str2.isEmpty()) {
                    this.mScheme = Constants.HTTPS;
                } else if (this.mPort == -1) {
                    if (Constants.HTTPS.equals(this.mScheme)) {
                        this.mPort = 443;
                    } else {
                        this.mPort = 80;
                    }
    ...
    public String toString() {
        String str;
        String str2 = "";
        if ((this.mPort == 443 || !Constants.HTTPS.equals(this.mScheme)) && (this.mPort == 80 || !Constants.HTTP.equals(this.mScheme))) {
            str = str2;
        } else {
