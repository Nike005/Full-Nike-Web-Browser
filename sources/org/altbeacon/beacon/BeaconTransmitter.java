...
public class BeaconTransmitter {
    ...
    public BeaconTransmitter(Context context, BeaconParser beaconParser) {
        ...
        if (bluetoothManager != null) {
            ...
            LogManager.m6046d(TAG, "new BeaconTransmitter constructed.  mbluetoothLeAdvertiser is %s", bluetoothLeAdvertiser);
            return;
        }
        LogManager.m6048e(TAG, "Failed to get BluetoothManager", new Object[0]);
    }
        ...
        if (beacon != null) {
            ...
            if (beaconParser != null) {
                ...
                for (int i2 = 0; i2 < beaconAdvertisementData.length; i2++) {
                    str = (str + String.format("%02X", new Object[]{Byte.valueOf(beaconAdvertisementData[i2])})) + StringUtils.SPACE;
                }
                ...
                LogManager.m6046d(TAG, "Starting advertising with ID1: %s ID2: %s ID3: %s and data: %s of size %s", objArr);
                try {
                    ...
                    LogManager.m6046d(TAG, "Started advertisement with callback: %s", getAdvertiseCallback());
                } catch (Exception e) {
                    LogManager.m6049e(e, TAG, "Cannot start advertising due to exception", new Object[0]);
                }
            } else {
    ...
    public void stopAdvertising() {
        if (!this.mStarted) {
            LogManager.m6046d(TAG, "Skipping stop advertising -- not started", new Object[0]);
            return;
        }
        LogManager.m6046d(TAG, "Stopping advertising with object %s", this.mBluetoothLeAdvertiser);
        this.mAdvertisingClientCallback = null;
        try {
            this.mBluetoothLeAdvertiser.stopAdvertising(getAdvertiseCallback());
        } catch (IllegalStateException unused) {
            LogManager.m6054w(TAG, "Bluetooth is turned off. Transmitter stop call failed.", new Object[0]);
        }
        this.mStarted = false;
    }
        ...
        if (this.mAdvertiseCallback == null) {
            this.mAdvertiseCallback = new AdvertiseCallback() {
                public void onStartFailure(int i) {
                    LogManager.m6048e(BeaconTransmitter.TAG, "Advertisement start failed, code: %s", Integer.valueOf(i));
                ...
                public void onStartSuccess(AdvertiseSettings advertiseSettings) {
                    LogManager.m6050i(BeaconTransmitter.TAG, "Advertisement start succeeded.", new Object[0]);
