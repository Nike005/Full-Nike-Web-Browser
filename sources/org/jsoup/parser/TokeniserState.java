...
enum TokeniserState {
    ...
    AfterDoctypeName {
        ...
        public void read(Tokeniser tokeniser, CharacterReader characterReader) {
            if (characterReader.isEmpty()) {
                ...
                tokeniser.emitDoctypePending();
                tokeniser.transition(Data);
            } else if (characterReader.matchesAny(9, 10, CharUtils.f3946CR, 12, ' ')) {
                characterReader.advance();
            } else if (characterReader.matches((char) Typography.greater)) {
                tokeniser.emitDoctypePending();
                tokeniser.advanceTransition(Data);
    ...
    CdataSection {
        /* access modifiers changed from: package-private */
        public void read(Tokeniser tokeniser, CharacterReader characterReader) {
            tokeniser.emit(characterReader.consumeTo(XmlSerializer.END_CDATA));
            characterReader.matchConsume(XmlSerializer.END_CDATA);
            tokeniser.transition(Data);
        }
    };
    ...
    static {
        ...
        attributeNameCharsSorted = new char[]{9, 10, CharUtils.f3946CR, 12, ' ', '/', '=', Typography.greater, 0, Typography.quote, '\'', Typography.less};
        attributeValueUnquoted = new char[]{9, 10, CharUtils.f3946CR, 12, ' ', Typography.amp, Typography.greater, 0, Typography.quote, '\'', Typography.less, '=', '`'};
