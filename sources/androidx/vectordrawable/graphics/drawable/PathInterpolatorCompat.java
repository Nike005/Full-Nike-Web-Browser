...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f4132mX;

    /* renamed from: mY */
    private float[] f4133mY;
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f4132mX = new float[min];
            this.f4133mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f4132mX[i2] = fArr[0];
                this.f4133mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f4132mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f4133mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f4132mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f4133mY[i3] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f4132mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb.append(this.f4132mX[0]);
            sb.append(",");
            sb.append(this.f4133mY[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f4132mX[i6]);
            sb.append(",");
            sb.append(this.f4133mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f4132mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f4132mX[i2]) {
                length = i2;
            } else {
                i = i2;
            }
        }
        float[] fArr = this.f4132mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f4133mY[i];
        }
        float[] fArr2 = this.f4133mY;
        float f3 = fArr2[i];
